...
namespace DAL
{
    public class ScoreListService
    {
        ...
        {
            string sql = "select stuCount=count(*),avgCSharp=avg(CSharp),avgDB=avg(SQLServer) from StudentList;";
            sql += "select absectCount=count(*) from Students where StudentId not in(select StudentId from StudentList)";
        ...
        {
            string sql = "select StudentName from Students where StudentId not in(select StudentId from StudentList)";
            SqlDataReader objReader = SQLHelper.GetReader(sql);
        ...
        {
            string sql = "select stuCount=count(*),avgCSharp=avg(CSharp),avgDB=avg(SQLServer) from StudentList ";
            sql += "inner join Students on Students.StudentId=StudentList.StudentId where ClassId={0};";
            sql += "select absentCount=count(*) from Students where StudentId not in";
            sql += "(select StudentId from StudentList) and ClassId={1}";
        ...
        public List<string> GetAbsentListByClassId(string classId)
        {
            ...
            sql += "(select StudentId from StudentList) and ClassId={0}";
        ...
        {
            string sql = "select Students.StudentId,StudentName,ClassName,Gender,PhoneNumber,CSharp,SQLServer from Students";
            sql += " inner join StudentClass on StudentClass.ClassId=Students.ClassId ";
            sql += " inner join StudentList on StudentList.StudentId=Students.StudentId ";
            if (className != null && className.Length != 0)
            {
                sql += " where ClassName='" + className + "'";
            }
        ...
        {
            string sql = "select Students.StudentId,StudentName,ClassName,Gender,PhoneNumber,CSharp,SQLServer";
            sql += " from Students";
            sql += " inner join StudentClass on StudentClass.ClassId=Students.ClassId ";
            sql += " inner join StudentList on StudentList.StudentId=Students.StudentId ";
